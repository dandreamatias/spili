...
const useComponent = (spili) => (file, data) => {
    const foo = new Function(
        'props, spili',
        'return ' + '`' + fs.readFileSync(path.join('template', 'components', `${file}.component.html`), 'utf8') + '`'
    );
...
const readArticle = (filePath) => {
    ...
    const article = {
        body,
        title: header.articleTitle,
        date: header.articleDate,
        description: header.articleDescription
    }
    const foo = new Function(
        `spili, article`,
        'return ' + '`' + fs.readFileSync(path.join('template', 'article.html'), 'utf8') + '`'
    );
    return foo(spiliInfo, article);
};
